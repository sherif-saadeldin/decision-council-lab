...
MODE_PROFILES: dict[DecisionMode, ModeProfile] = {
    DecisionMode.FAST_ANSWER: ModeProfile(
        ...
        label="Quick directional answer",
        routing_mode="economy",
        debate_rounds=0,
        slot_emphasis="cheapest viable chair, no debate",
        description="Fast recommendation with minimal overhead.",
    ),
    DecisionMode.DEEP_ANALYSIS: ModeProfile(
        DecisionMode.DEEP_ANALYSIS,
        label="Careful strategic review",
        routing_mode="balanced",
        debate_rounds=2,
        slot_emphasis="full council, multi-model debate",
        description="Thoughtful strategic analysis with broader perspective.",
    ),
    DecisionMode.PRESSURE_TEST: ModeProfile(
        ...
        description="Challenge assumptions and expose hidden weaknesses.",
    ),
    DecisionMode.BUILD_PLAN: ModeProfile(
        DecisionMode.BUILD_PLAN,
        label="Turn this into an execution plan",
        routing_mode="balanced",
        debate_rounds=1,
        slot_emphasis="operator emphasized",
        description="Translate strategy into sequenced, practical next steps.",
    ),
    DecisionMode.RISK_REVIEW: ModeProfile(
        DecisionMode.RISK_REVIEW,
        label="Risk and downside scan",
        routing_mode="balanced",
        debate_rounds=1,
        slot_emphasis="risk-led, kill criteria heavy",
        description="Surface key risks, failure modes, and kill criteria.",
    ),
    DecisionMode.EXECUTION_ROADMAP: ModeProfile(
        DecisionMode.EXECUTION_ROADMAP,
        label="Execution roadmap with momentum",
        routing_mode="balanced",
        debate_rounds=1,
        slot_emphasis="operator + implementation focus",
        description="Create a momentum-friendly roadmap and delivery path.",
    ),
}
...
_MODE_ALIASES: dict[str, DecisionMode] = {
    ...
    "directional": DecisionMode.FAST_ANSWER,
    ...
    "careful": DecisionMode.DEEP_ANALYSIS,
    "strategic": DecisionMode.DEEP_ANALYSIS,
    ...
    "stress-test": DecisionMode.PRESSURE_TEST,
    "assumptions": DecisionMode.PRESSURE_TEST,
